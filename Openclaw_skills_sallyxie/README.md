...
`video-to-gif` converts a local video clip into a GIF using `ffmpeg`, with support for frame rate, width, start time, and duration.
...
Copy and paste this full command into Terminal, then press Enter:
...
#### Option 2: Ask OpenClaw in Feishu or DingTalk

You can send a message to your OpenClaw bot in Feishu or DingTalk, for example:
...
- `video-to-gif` adds a reusable video-to-GIF conversion workflow backed by `ffmpeg`
...
`video-to-gif` 这个 Skills 的作用是使用 `ffmpeg` 将本地视频片段转换为 GIF，并支持帧率、宽度、开始时间、时长等参数。

### `video-to-gif` 的安装方式

这个 Skills 也提供两种适合新手的安装方式。
...
#### 方式 2：直接发给飞书或钉钉里的 OpenClaw 机器人

你也可以直接在飞书或钉钉里对 OpenClaw 机器人说：
